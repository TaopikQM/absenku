...
// }, [mediaRecorder]);
  ...
  // Fungsi untuk mengambil data batasan waktu dari Firebase
function getKominfoBatasan(callback) {
  const dbRef = ref(rtdb,'kp/magang/batasan');
  ...
  dbRef
    ...
    .once("value", (snapshot) => {
      if (snapshot.exists()) {
        snapshot.forEach((childSnapshot) => {
          const data = childSnapshot.val();
          callback(null, data); // Mengembalikan data batasan dari Firebase
        });
      } else {
        callback("Data tidak ditemukan", null);
      }
  ...
  const isWithinAllowedTime = () => {
    ...
    const day = now.getDay(); // 0 = Sunday, 1 = Monday, ..., 6 = Saturday
     ...
     return day >= 1 && day <= 5 && (hours >= 4 && (hours < 24 || (hours === 24 && minutes === 0)));
  };
  ...
  //   const hours = now.getUTCHours() + 7; // Convert to WIB (UTC+7)
  //   const minutes = now.getUTCMinutes();
  //   const [timein_min, timein_max] = [batasan.timein_min, batasan.timein_max];
    
  //     // Membandingkan jam dan menit
  //     const [startHour, startMinute] = timein_min.split(':').map(Number);
  //     const [endHour, endMinute] = timein_max.split(':').map(Number);
  //     return (
  ...
  // const isWithinAllowedTimeForPulang = (batasan) => {
  //   // const dbRef = ref(rtdb,'kp/magang/batasan');
  ...
  //     timeout_min = batasan.absensiId.schedule.monday_thursday.timeout_min;
  //     timeout_max = batasan.absensiId.schedule.monday_thursday.timeout_max;
  //   } else if (day === 5) { // Jumat
  //     timeout_min = batasan.schedule.friday.timeout_min;
  //     timeout_max = batasan.schedule.friday.timeout_max;
  //   } else {
  //     return false; // Sabtu dan Minggu tidak diperbolehkan
  //   }
  
  //   // Membandingkan jam
  //   const [startHour] = timeout_min.split(':').map(Number);
  //   const [endHour] = timeout_max.split(':').map(Number);
  ...
  const handleScan = async () => {
    ...
    if (distanceToKominfo > 3) {
      alert('Lokasi Anda tidak sesuai dengan lokasi kantor.');
      setLoading(false);
      return;
    }

    const kpAbsenRef = ref(rtdb, 'kp/magang/absenu/');
    ...
    if (found && matchedName) { 
      ...
      if (usersData) {
        ...
        if (userDetails && userDetails.status === 'ACTIVE') {
          // Stop the recording before saving attendance
          mediaRecorder.stop();
  ...
  const saveAttendance = async (userId, userName) => {
    ...
    const attendanceRef = ref(rtdb, `kp/magang/absenu/${userId}/attendance/${formattedDate}`);
    ...
    const absenhRef = ref(rtdb, `kp/magang/absenuj/${formattedDate}/attendance`);
    ...
    const absensiId = newAbsenhRef.key;
    sessionStorage.setItem('absensiId', absensiId);
  ...
  //       if (userDetails && userDetails.status === 'ACTIVE') {
  //         // Stop the recording and save attendance
  //         mediaRecorder.stop();

  //         // Wait for video recording to stop and blob to be available
  //         mediaRecorder.onstop = async () => {
  //           if (videoBlob) {
  //             const absensiId = await saveAttendance(currentUserId, matchedName);
  //             await uploadVideo(absensiId, videoBlob);
  ...
  // const uploadVideo = async (absensiId, videoBlob) => {
  //   const videoFileName = `${absensiId}.mp4`;
  //   const videoStorageRef = storageRef(storage, `kp/videos/${videoFileName}`);

  //   try {
  //     const snapshot = await uploadBytes(videoStorageRef, videoBlob);
  //     const videoURL = await getDownloadURL(snapshot.ref);

  //     const attendanceRef = ref(rtdb, `kp/magang/absenuj/${absensiId}`);
  //     await set(attendanceRef, { video: videoURL }, { merge: true }); // Save video URL
  //     console.log('Video berhasil di-upload dan URL disimpan.');
  //   } catch (error) {
  //     console.error('Gagal upload video:', error);
  //   }
...
//   const uploadVideo = async (absensiId, videoBlob) => {
//     const videoFileName = `${absensiId}.mp4`;
//     const videoStorageRef = storageRef(storage, `videos/absenuj/${videoFileName}`);
...
//       // Simpan URL video ke dalam absensi
//       const attendanceRef = ref(rtdb, `kp/magang/absenuj/${absensiId}/video`);
//       await set(attendanceRef, {video: videoURL }, { merge: true });
//       console.log('Video berhasil di-upload dan URL disimpan.');
//     } catch (error) {
//       console.error('Gagal upload video:', error);
//     }
//   };
  ...
  //   const imageBlob = await new Promise(resolve => canvas.toBlob(resolve, 'mp4/avi/blob'));

  //   // Upload gambar ke Firebase Storage
  //   const date = new Date();
  //   const formattedDate = date.toISOString().split('T')[0];
  //   const storageRefPath = storageRef(storage, `kp/magang/absenuj/${userId}/attendance/${formattedDate}`);
  //   const snapshot = await uploadBytes(storageRefPath, imageBlob);
  //   const imageUrl = await getDownloadURL(snapshot.ref);

  //   // Simpan data absensi ke Firebase Realtime Database
  //   const absensiId = await saveAttendance(userId, userName, imageUrl);
    
  //   setCameraStarted(false);
  //   setLoading(false);
  // };

  // const saveAttendance = async (userId, userName, imageUrl) => {
  //   const date = new Date();
  //   const formattedDate = date.toISOString().split('T')[0];
  //   const hours = date.getUTCHours() + 7; // WIB
  //   const minutes = date.getUTCMinutes();
  //   const formattedTime = `${hours.toString().padStart(2, '0')}:${minutes.toString().padStart(2, '0')} WIB`;
  ...
  //     timot: '',
  //     kegiatan: '',
  //     imageUrl, // URL gambar juga disimpan di tabel absenuj
  //   });
  ...
  const saveAttendance1 = async (userId,  userName, kegiatan) => {
    ...
    const absenhRef = ref(rtdb, `kp/magang/absenuj/${formattedDate}/attendance/${absensiId}`);
    //const newAbsenhRef = push(absenhRef);
    ...
    //alert('Absensi pulang berhasil.');
    // const absenId = absenhRef.key;
    // sessionStorage.setItem('absenId', absenId);
    //alert('Absensi berhasil disimpan!');
    return absensiId;
  };
  ...
  const handleOutOfTownAttendance = async (userId, userName) => {
    ...
    // Simpan data absenh di tabel absenh
    const absenhRef = ref(rtdb, `kp/magang/absenuj/${formattedDate}/attendance/${userName}`);
    await set(absenhRef, {
        name: userName,
        idabsen: userId,
        location,
    ...
    if (descriptorExists) {
        ...
        if (nameExists) {
          if (isUserActive) {
            ...
            const kpUsersRef = ref(rtdb, `kp/magang/absenu/${newUser.id}`);
